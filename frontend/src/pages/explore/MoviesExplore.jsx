...
const MoviesExplore = memo(() => {
  ...
  const genres = [
    ...
    'Sci-Fi', 'Fantasy', 'Adventure', 'Western', 'Crime', 'Mystery'
  ]
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const [trendingMovies, popularMovies, topRated] = await Promise.all([
        ...
        const movies = safeList(trendingMovies)
        ...
        const moviesWithImages = movies.filter(hasImage)
        
        setAllContent(moviesWithImages)
        setTrendingContent(moviesWithImages.slice(0, 10))
        setTopRatedContent(safeList(topRated).filter(hasImage).slice(5, 25))
        setNewReleases(moviesWithImages.slice(10, 30))

        // Generate hot news from trending content
        const news = moviesWithImages.slice(0, 3).map((item, idx) => ({
          id: idx + 1,
        ...
        if (isAuthenticated) {
          try {
            ...
            const movieTvItems = allItems.filter(item => 
              item.mediaType === 'movie' || item.mediaType === 'tv'
            )
            setContinueWatching(movieTvItems.slice(0, 4))
            
            // Because You Watched - get recommendations from last watched
            const lastWatched = movieTvItems[0]
            if (lastWatched) {
              try {
                const details = await mediaService.getDetails(
                  lastWatched.mediaType,
                  lastWatched.mediaId
                )
                const recommendations = details.recommendations || details.similar || []
                const recs = safeList(recommendations)
                setBecauseYouWatched(recs.slice(0, 20))
              } catch (e) {
  ...
  const applyFilters = useCallback(() => {
    ...
    if (mediaType !== 'all') {
      filtered = filtered.filter(item => {
        const type = item.type || item.media_type || item.mediaType || 'movie'
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'rating':
          aVal = a.vote_average || a.rating || 0
          bVal = b.vote_average || b.rating || 0
          break
        case 'release_date':
  ...
  }, [allContent, selectedGenres, yearRange, ratingRange, mediaType, sortBy, sortOrder])
  ...
  const activeFilters = useMemo(() => {
    ...
    if (mediaType !== 'all') {
      filters.push({ type: 'mediaType', label: 'Type', value: mediaType })
    }
    return filters
  }, [selectedGenres, yearRange, ratingRange, mediaType])
...
MoviesExplore.displayName = 'MoviesExplore'
