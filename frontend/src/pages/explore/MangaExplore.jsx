...
const MangaExplore = memo(() => {
  ...
  const genres = [
    ...
    'Romance', 'Sci-Fi', 'Slice of Life', 'Sports', 'Supernatural', 'Thriller', 'Manhwa'
  ]
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const [trendingManga, popularManga, topRated] = await Promise.all([
        ...
        const manga = safeList(trendingManga)
        ...
        const mangaWithImages = manga.filter(hasImage)
        
        setAllContent(mangaWithImages)
        setTrendingContent(mangaWithImages.slice(0, 20))
        setTopRatedContent(safeList(topRated).filter(hasImage).slice(5, 25))
        setNewReleases(mangaWithImages.slice(10, 30))

        // Generate hot news from trending content
        const news = mangaWithImages.slice(0, 3).map((item, idx) => ({
          id: idx + 1,
        ...
        if (isAuthenticated) {
          try {
            ...
            const mangaItems = allItems.filter(item => 
              item.mediaType === 'manga' || item.mediaType === 'manhwa'
            )
            setContinueWatching(mangaItems.slice(0, 4))
  ...
  const applyFilters = useCallback(() => {
    ...
    if (mediaType !== 'all') {
      filtered = filtered.filter(item => {
        const type = item.type || item.media_type || item.mediaType || 'manga'
        return type === mediaType || (mediaType === 'manga' && (type === 'manga' || type === 'manhwa'))
      })
    }
...
MangaExplore.displayName = 'MangaExplore'
